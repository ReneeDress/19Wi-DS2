...
public:
	...
	MulAdjNetworkArc<WeightType>* G
class MulAdjUndirGraphetNextarc(const MulAdjNetworkArc<WeightType>& a1, int v) const;	//边a1关于v的下一条边
	...
	void DeleteVex(int v1,int m[5][5]);
	void DeleteVex(ElemType e1, int m[5][5]);	
...
template <class ElemType, class WeightType>
void MulAdjUndirGraph<ElemType, WeightType>::DeleteVex(int v, int m[5][5])
{
	if (v == vexNum)
		cout << "图中不存在要删除的顶点!" << endl; // 异常
	for (int i = 0; i < vexNum; i++)
	{
		if (i != v)
		{
...
template <class ElemType, class WeightType>
void MulAdjUndirGraph<ElemType, WeightType>::DeleteVex(ElemType e, int m[5][5])
{
	v = GetOrder(e);
	if (v == vexNum)
		cout << "图中不存在要删除的顶点!" << endl; // 异常
