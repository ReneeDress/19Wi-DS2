...
public:
	...
	void DeleteVex(int v1,int m[0][0]);
	void DeleteVex(ElemType e1, int m[0][0]);	
...
template <class ElemType, class WeightType>
void MulAdjUndirGraph<ElemType, WeightType>::DeleteVex(int v, int m[0][0])
{
	if (v >= vexNum)
...
template <class ElemType, class WeightType>
void MulAdjUndirGraph<ElemType, WeightType>::DeleteVex(ElemType e, int m[0][0])
{
	v = GetOrder(e);
	if (v >= vexNum)
		{cout << "图中不存在要删除的顶点!" << endl; return;}// 异常
