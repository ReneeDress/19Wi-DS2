...
#define infinity DEFAULT_INFINITY
...
// 邻接矩阵类型的方法声明:
	...
	void ConnectedComponentHelp(const AdjMatrixUndirGraph<ElemType> &g, int v);
	int ConnectedComponent(const AdjMatrixUndirGraph<ElemType> &g);
...
// 操作结果: 显示邻接矩阵无向图
{
	...
	for (int v = 0; v < vexNum; v++)	{
		cout << vertexes[v];
		for (int u = 0; u < vexNum; u++)
 			if (arcs[v][u] == infinity || arcs[v][u] == 0) cout << "\t" << "∞";
 			else {cout << "\t" << arcs[v][u];}
		cout << endl;
	}
}
...
template <class ElemType>
int AdjMatrixUndirGraph<ElemType>::CountDegree(int v)
// 操作结果: 显示顶点v的度数
{
	if (v < 0 || v >= vexNum)
       throw Error("v不合法!");// 抛出异常
	...
	for (int u = 0; u < vexNum; u++)
	{
		if (arcs[v][u] != 0 && arcs[v][u] != infinity) cnt++;
		if (u == vexNum - 1) return cnt;		
	}
...
template <class ElemType>
void AdjMatrixUndirGraph<ElemType>::DFS(const AdjMatrixUndirGraph<ElemType> &g, int v, void (*Visit)(const ElemType &))
{
	ElemType e;
	g.SetTag(v, VISITED);
	g.GetElem(v,e);
	...
	{
		if (g.GetTag(w) == UNVISITED) DFS(g,w,Visit);
	}
}

template <class ElemType>
void AdjMatrixUndirGraph<ElemType>::DFSTraverse(const AdjMatrixUndirGraph<ElemType> &g, void (*Visit)(const ElemType &))
{
	int v;
	for (v = 0; v < g.GetVexNum(); v++) g.SetTag(v, UNVISITED);
	for (v = 0; v < g.GetVexNum(); v++)
	{
		if (g.GetTag(v) == UNVISITED) DFS(g, v, Visit);
	}
}

template <class ElemType>
void AdjMatrixUndirGraph<ElemType>::ConnectedComponentHelp(const AdjMatrixUndirGraph<ElemType> &g, int v)
{
	ElemType e;
	g.SetTag(v, VISITED);
	g.GetElem(v,e);
	for (int w = g.FirstAdjVex(v); w != -1; w = g.NextAdjVex(v,w))
	{
		...
		if (g.GetTag(w) == UNVISITED) ConnectedComponentHelp(g,w);
	}
}

template <class ElemType>
int AdjMatrixUndirGraph<ElemType>::ConnectedComponent(const AdjMatrixUndirGraph<ElemType> &g)
// 操作结果: 显示图的连通分量数目
{
	int v;
	int cnt = 0;
	...
	for (v = 0; v < g.GetVexNum(); v++)
	{
		if (g.GetTag(v) == UNVISITED) cnt++; ConnectedComponentHelp(g,v);
	}
	return cnt;
}
